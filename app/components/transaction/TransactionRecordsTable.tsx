...
const TransactionRecordsTable: React.FC<TransactionRecordsTableProps> = ({
  ...
  const filterByPaymentType = (transaction: Transaction) => {
    const filter = filterPeriod?.find(
      (filter) => filter.type === "paymentType"
    );
    if (!filter) {
      return true;
    }
    ...
    return filter.value.includes(transaction.type);
  };
